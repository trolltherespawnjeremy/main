(function() {
  ...
  var allowed_balansers = ['filmix','filmixtv','rezka','kinotochka','collaps','collaps-dash','hdvb','veoveo','vkvideo'];

  var balanser_names = {'filmix':'Filmix','filmixtv':'Filmix TV','rezka':'Rezka HD','kinotochka':'Kinotochka','collaps':'Collaps','collaps-dash':'Collaps HD','hdvb':'HDVB','veoveo':'VeoVeo','vkvideo':'VK Video'};
...
window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
  ...
  function rchRun(json, call) {
    if (typeof NativeWsClient == 'undefined') {
      Lampa.Utils.putScript(["http://rc.bwa.ad/js/nws-client-es5.js?v07032026"], function() {}, false, function() {
        rchInvoke(json, call);
      }, true);
    } else {
      rchInvoke(json, call);
  ...
  function component(object) {
    ...
    this.initialize = function() {
      ...
      filter.onSelect = function(type, a, b) {
        if (type == 'filter') {
          if (a.reset) {
            ...
            if (a.stype == 'source') {
              // Balancer selected from filter panel
              Lampa.Select.close();
              object.lampac_custom_select = b.index;
              _this.changeBalanser(b.index);
            } else {
      ...
      filter.render().find('.filter--filter span').text(Lampa.Lang.translate('title_filter'));

      // MutationObserver
      var observer = new MutationObserver(function() {
        $('.selectbox__title').each(function() {
          var t = $(this).text();
          if (t.indexOf('орт') !== -1) {
            $(this).text(Lampa.Lang.translate('lampac_balanser'));
          } else if (t.indexOf('ильтр') !== -1) {
            $(this).text(Lampa.Lang.translate('title_filter'));
          }
        });
      });
    ...
    this.startSource = function(json) {
      return new Promise(function(resolve, reject) {
        json = json.filter(function(j) { return allowed_balansers.indexOf(balanserName(j)) !== -1; });
        json.forEach(function(j) {
          var name = balanserName(j);
          sources[name] = {
            url: j.url,
            name: balanser_names[name] || j.name,
      ...
      return new Promise(function(resolve, reject) {
        ...
        var fin = function fin(call) {
          ...
          network.silent(account(url), function(json) {
            ...
            filter_sources = [];
            sources = {};
            json.online.filter(function(j) { return allowed_balansers.indexOf(balanserName(j)) !== -1; }).forEach(function(j) {
              var name = balanserName(j);
              sources[name] = {
                url: j.url,
                name: balanser_names[name] || j.name,
    ...
    this.filter = function(filter_items, choice) {
      ...
      // Add balancer/source first
      if (filter_sources.length) {
        var sourceItems = [];
        filter_sources.forEach(function(e) {
          sourceItems.push({
            title: sources[e].name,
            selected: e == balanser,
            index: e,
        ...
        select.push({
          ...
          subtitle: sources[balanser] ? sources[balanser].name : balanser,
          items: sourceItems,
          stype: 'source'
        });
      }
    ...
    this.contextMenu = function(params) {
      params.html.on('hover:long', function() {
        function show(extra) {
          ...
          menu.push({
            title: Lampa.Lang.translate('lampac_change_balanser'),
            changebalanser: true
          });
          ...
          Lampa.Select.show({
            ...
            onSelect: function onSelect(a) {
              ...
              if (a.changebalanser) {
                ...
                setTimeout(function() {
                  $('.selectbox__title, .filter__title').filter(function() {
                    return $(this).text().indexOf('орт') !== -1;
  ...
  function startPlugin() {
    ...
    var manifst = {
      ...
      onContextLauch: function onContextLauch(object) {
        resetTemplates();
        Lampa.Component.add('reficohd', component);
    ...
    Lampa.Lang.add({
      ...
      lampac_video: {
        ru: 'Video', en: 'Video', uk: 'Відео', zh: '视频'
      },
      lampac_no_watch_history: {
        ru: 'Nav skatīšanās vēstures', en: 'No watch history', ua: 'Немає історії перегляду', zh: '没有浏览历史'
      },
      lampac_nolink: {
        ru: 'Neizdevās saņemt video adresi', uk: 'Неможливо отримати посилання', en: 'Failed to fetch link', zh: '获取链接失败'
      },
      lampac_balanser: {
        ru: 'Balanseris', uk: 'Джерело', en: 'Source', zh: '来源'
      },
      helper_online_file: {
        ru: 'Turiet pogu "OK" lai parādītu izvēlni',
        uk: 'Утримуйте клавішу "ОК" для виклику контекстного меню',
        en: 'Hold "OK" to open context menu',
        zh: '按住"确定"键调出上下文菜单'
      },
      title_online: {
        ru: 'Skatīties', uk: 'Онлайн', en: 'Online', zh: '在线的'
      },
      lampac_voice_subscribe: {
        ru: 'Pierakstīties tulkojumam', uk: 'Підписатися на переклад', en: 'Subscribe to translation', zh: '订阅翻译'
      },
      lampac_voice_success: {
        ru: 'Jūs veiksmīgi pierakstījāties', uk: 'Ви успішно підписалися', en: 'Successfully subscribed', zh: '您已成功订阅'
      },
      lampac_voice_error: {
        ru: 'Radās kļūda', uk: 'Виникла помилка', en: 'An error occurred', zh: '发生了错误'
      },
      lampac_clear_all_marks: {
        ru: 'Noņemt visas atzīmes', uk: 'Очистити всі мітки', en: 'Clear all labels', zh: '清除所有标签'
      },
      lampac_clear_all_timecodes: {
        ru: 'Noņemt visas laika atzīmes', uk: 'Очистити всі тайм-коди', en: 'Clear all timecodes', zh: '清除所有时间代码'
      },
      lampac_change_balanser: {
        ru: 'Nomainīt balanseri', uk: 'Змінити джерело', en: 'Change source', zh: '更改来源'
      },
      lampac_balanser_dont_work: {
        ru: 'Atbilstoši rezultāti netika atrasti', uk: 'Пошук не дав результатів', en: 'No results found', zh: '搜索未返回任何结果'
      },
      lampac_balanser_timeout: {
        ...
        en: 'Source will switch automatically in <span class="timeout">10</span> seconds.',
        zh: '来源将在<span class="timeout">10</span>秒内自动切换。'
      },
      lampac_does_not_answer_text: {
        ru: 'Atbilstoši rezultāti nav atrasti', uk: 'Пошук не дав результатів', en: 'No results found', zh: '搜索未返回任何结果'
      },
      // Override Lampa built-in strings
      title_filter: {
        ru: 'Filtrs', en: 'Filter', uk: 'Фільтр', zh: '筛选'
      },
      torrent_serial_season: {
        ru: 'Sezona', en: 'Season', uk: 'Сезон', zh: '季'
      },
      torrent_serial_episode: {
        ru: 'Sērija', en: 'Episode', uk: 'Серія', zh: '集'
      },
      torrent_parser_voice: {
        ru: 'Tulkojums', en: 'Voice', uk: 'Переклад', zh: '配音'
      },
      torrent_parser_reset: {
        ru: 'Atiestatīt filtru', en: 'Reset filter', uk: 'Скинути фільтр', zh: '重置'
      },
      torrent_parser_label_title: {
        ru: 'Atzīmēt kā skatītu', en: 'Mark as watched', uk: 'Позначити', zh: '标记'
      },
      torrent_parser_label_cancel_title: {
        ru: 'Noņemt atzīmi', en: 'Unmark', uk: 'Зняти позначку', zh: '取消标记'
      },
      title_action: {
        ru: 'Darbība', en: 'Action', uk: 'Дія', zh: '操作'
      },
      cancel: {
        ru: 'Atcelt', en: 'Cancel', uk: 'Скасувати', zh: '取消'
      },
      more: {
        ru: 'Vairāk', en: 'More', uk: 'Більше', zh: '更多'
      },
      time_reset: {
        ru: 'Dzēst taimkodu', en: 'Clear timecode', uk: 'Скинути таймкод', zh: '重置时间'
      },
      copy_link: {
        ru: 'Kopēt adresi', en: 'Copy link', uk: 'Копіювати посилання', zh: '复制链接'
      },
      player_lauch: {
        ru: 'Atvērt', en: 'Launch player', uk: 'Відкрити', zh: '启动'
      },
      settings_rest_source: {
        ru: 'Avots', en: 'Source', uk: 'Джерело', zh: '来源'
      },
      title_error: {
        ru: 'Kļūda', en: 'Error', uk: 'Помилка', zh: '错误'
      },
      full_episode_days_left: {
        ru: 'Atlikušas dienas', en: 'Days left', uk: 'Днів залишилось', zh: '剩余天数'
      },
      empty_title_two: {
        ru: 'Nav rezultātu', en: 'No results', uk: 'Немає результатів', zh: '无结果'
      },
      empty_text: {
        ru: 'Nekas netika atrasts', en: 'Nothing was found', uk: 'Нічого не знайдено', zh: '未找到任何内容'
      }
    });
    ...
    Lampa.Component.add('reficohd', component); //то же самое
    resetTemplates();
    ...
    if (Lampa.Manifest.app_digital >= 177) {
        var balansers_sync = ['filmix', 'filmixtv', 'rezka', 'kinotochka', 'collaps', 'collaps-dash', 'hdvb', 'veoveo', 'vkvideo'];
  ...
  if (!window.reficohd_started) startPlugin();
